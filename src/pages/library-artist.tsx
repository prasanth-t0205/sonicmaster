...
function ArtistDetailContent() {
  ...
  const rawArtistName = searchParams.get("name");
  const artistName = rawArtistName
    ? (() => {
        try {
          return decodeURIComponent(rawArtistName);
        } catch {
          return rawArtistName;
        }
      })()
    : "";
  ...
  const isCurrentPlaylist =
    currentSong && artistSongs.some((s) => s.path === currentSong.path);
          ...
          <div className="w-48 h-48 rounded-3xl bg-linear-to-tr from-primary/20 via-primary/5 to-transparent flex items-center justify-center shrink-0 border border-white/5 shadow-2xl backdrop-blur-sm group hover:scale-[1.02] transition-transform duration-500 overflow-hidden relative">
            {artistSongs[0] ? (
              <div className="absolute inset-0">
                <TrackArt
                  path={artistSongs[0].path}
                  hasArt={artistSongs[0].hasArt}
