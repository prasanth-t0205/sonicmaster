...
function GenreDetailContent() {
  ...
  const genreName = rawGenreName
    ? (() => {
        try {
          return decodeURIComponent(rawGenreName);
        } catch {
          return rawGenreName;
        }
      })()
  ...
  const isCurrentPlaylist =
    currentSong && genreSongs.some((s) => s.path === currentSong.path);
  ...
  const handleAddAllToQueue = () => {
    genreSongs.forEach((song) => addToQueue(song));
  };

  if (!genreName) return null;

  const totalDuration = genreSongs.reduce(
          ...
          <div className="w-48 h-48 rounded-3xl bg-linear-to-tr from-primary/20 via-primary/5 to-transparent flex items-center justify-center shrink-0 border border-white/5 shadow-2xl backdrop-blur-sm group hover:scale-[1.02] transition-transform duration-500 overflow-hidden relative">
            {genreSongs[0] ? (
              <div className="absolute inset-0">
                <TrackArt
                  path={genreSongs[0].path}
                  hasArt={genreSongs[0].hasArt}
